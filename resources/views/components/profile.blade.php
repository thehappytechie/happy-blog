<div class="relative" x-data="{ open: false }">
    <button @click="open = ! open"> <span class="flex items-center lg:flex lg:items-center mt-1.5">
            ...
            <img class="h-7 w-7 rounded-full bg-gray-50"
                ...
                alt="">
            <span class="ml-2 text-sm font-medium leading-6 text-gray-800" aria-hidden="true">{{ Auth::user()->name }}</span>
    ...
    <div x-show="open" @click.outside="open = false"
        ...
        <a href="{{ route('user.update.profile', Auth::user()->id) }}" class="block px-3 py-1 text-sm leading-6 text-gray-800" role="menuitem" tabindex="-1"
            id="user-menu-item-0">Your profile</a>
        <a href="{{ route('user.update.password') }}" class="divide-y divide-gray-100 block px-3 py-1 text-sm leading-6 text-gray-800" role="menuitem"
